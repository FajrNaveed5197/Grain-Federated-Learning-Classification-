...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Full-data FedAvg experiment initialized from ResNet18 V3."
    )
...
def local_class_weights(dataset: ManifestImageDataset) -> torch.Tensor:
    targets = dataset.dataframe["label"].map(CLASS_TO_ID).to_numpy()
...
def save_state(
    ...
    with open(output_dir / "metrics.json", "w", encoding="utf-8") as handle:
        json.dump(
            {
                "experiment": "fedavg_full_iid_resnet18_v3",
                "completed_round": completed_round,
                "history": history,
            },
            handle,
...
def main() -> None:
    ...
    if not client_manifests:
        raise FileNotFoundError(f"No client manifests found: {client_dir}")
